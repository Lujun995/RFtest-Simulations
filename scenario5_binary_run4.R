  ...
  for(j in 1:length(n)){
    ...
    for(k in 1:length(signal)){
      for(l in 1){
        ...
        #RF omnibus test
        pv1 <- randomForestTest_parallel_omnibus2(comm = x , meta.data = data.frame(y=factor(y[1:n[j],h,j,k,l])), tree=tree, 
        ...
        # #MiHC
        # pv5 <- MiHC::MiHC(y=y[1:n[j],h,j,k,l], otu.tab=x, tree = tree, model="binomial", n.perm=999)
        # pv5$ada.pvs["MiHC"] -> pv.mat[h,c("MiHC"),j,k,l]
      }
    }
  }
  if(h %% round(iter/100) == 0) save.image(file="/hpc/home/lz197/RFomnibus/s5_b_run4.RData")
}
save.image(file="/hpc/home/lz197/RFomnibus/s5_b_run4.RData")
