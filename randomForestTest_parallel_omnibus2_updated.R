# 2021/02/13 using the p.value as the tested statsitcs
# enbale to return p.values of multiple methods
# 2021/02/27 Include only non-phylo and phylo methods
...
#permutate the covariate
covariate_generation<-function(y,covariate,y.new){
  if(!(class(y) %in% c("numeric","integer","factor"))) stop("y is not an intrger, numeric or factor variable.")
  if(!(class(y.new) %in% c("numeric","integer","factor"))) stop("y.new is not an intrger, numeric or factor variable.")
  cov.p<-matrix(NA, nrow = nrow(covariate), ncol=ncol(covariate))
  for(i in 1:ncol(covariate)){
    if (class(covariate[,i]) %in% c("numeric","integer")){
      temp=y
      model=lm(covariate[,i]~temp)
      temp=y.new
      cov.p[,i]<-predict(model,newdata=data.frame(temp=temp))+sample(model$residuals) #bootstrap or permuatation?
    } else 
      if (class(covariate[,i]) %in% c("factor")){
        temp=y
        model=glm(covariate[,i]~temp,family = "binomial")
        temp=y.new
        cov.p[,i]<-sapply(
          X=predict(model,newdata = data.frame(temp=temp), type = "response"),
          FUN=rbinom,
          n=1, size=1) #covariate.p may be multi-level
      } else stop("Covariate is not an intrger, numeric or factor variable.")
  }
  return(cov.p)
}

# This is the major function
randomForestTest_parallel_omnibus2 <- 
  function (comm, meta.data, response.var, status.var = NULL, adjust.vars = NULL, 
            ...
            method = c('omnibus','weighted','unweighted'), tree = NULL, probability = TRUE,
            feature.no = 0.25 * ncol(x),  test.type = c('Training', 'OOB', '0.632', '0.632+'),
  ...
  if(method=="omnibus"){
    ...
    for(i in 1:3){# take average of 3 runs to estimate stat.o
      #weighted
  ...
  clusterExport(cl, c("ranger","calculateError", "covariate_generation","calculate.cum")) 
...
#      if (i %% 100 == 0) cat('.') 
#    }
    # permuatate the response var, Simulating the covariates
    meta.perm<-meta.data
    meta.perm[,response.var]<-sample(meta.data[, response.var])
    if(!is.null(adjust.vars)){
      meta.perm[,adjust.vars] <-covariate_generation(y = meta.data[, response.var], covariate = meta.data[,adjust.vars, drop = FALSE],
                                                     y.new=meta.perm[,response.var])
    }
...
#  cat('\n')
  ...
  return(list(call = call, method = method, p.value.perm = p.value.perm, p.value.pearson = p.value.pearson, 
              pearson.para = pearson.para, stat.o = stat.o, w = w))
}
