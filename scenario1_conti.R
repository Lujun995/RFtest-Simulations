#Source code for the scenario 1 (clear)
#beta= 0, 0.25, 0.5, 0.75, 1
#n=50
#signal density=5%, 15%
#five methods=RFtest, MiRKAT, OMiAT, aMiSPU
#two signal types=randomly selected and phylogenetically clustered OTUs
...
beta0= 10 #to largely avoid negative y
...
set.seed(seed=1123456) #another random seed not equal to the one in s1_b
...
for(h in 1:iter){
  ...
  for(k in 1:length(signal)){
	...
	names(signal.tips)<-paste("Density=",Density,sep="")
...
save.image(file="/hpc/home/lz197/RFomnibus/s1_c.RData")


cores = 10 #to modify according to computer

#set.seed(seed=2345611) #different from s1_b and above
for(h in 1:iter){
  ...
  for(k in 1:length(signal)){
	for(l in 1:length(Density)){
		for(j in 1:length(beta)){
			...
			#obtain y=y[ ,h,j,k,l]
			#RF omnibus test
			pv1 <- randomForestTest(comm = x , meta.data = data.frame(y=y[ ,h,j,k,l]), tree=tree,
  ...
  if(h %% round(iter/100) == 0) save.image(file="/hpc/home/lz197/RFomnibus/s1_c.RData")
}
save.image(file="/hpc/home/lz197/RFomnibus/s1_c.Rdata")