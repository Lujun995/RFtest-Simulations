#Scenario 4: different lineages and different linking function, binary outcomes
#beta=1
#n=50
...
for(h in 1:iter){
  ...
  for(k in 1:length(linking.method)){
	for(l in 1:length(lineage.nodes)){
		...
		#RF omnibus test
		pv1 <- randomForestTest_parallel_omnibus2(comm = x , meta.data = data.frame(y=factor(y[ ,h,k,l])), tree=tree, 
		...
		# #MiHC
		# pv5 <- MiHC::MiHC(y=y[ ,h,k,l], otu.tab=x, tree = tree, model="binomial", n.perm=999)
