...
randomForestTest_parallel_omnibus2 <- 
  ...
  if(!is.null(adjust.vars)){ #fitting the model outside the loop to save time
    ...
    if(prediction.type %in% c('Regression')) {
      ...
      if(prediction.type %in% 'Classification') {
        ...
        res <- residuals(model,type="response")
        #the first level of response.var is 0 in logistic regression. the residuals of level 0 are negative.
        #therefore, negative values would move towards the 0 or the first level.
        response.adj <- factor(rbinom(n=nrow(meta.data), size = 1, prob = (res+1)/2), levels = c(0,1)) 
        levels(response.adj) <- levels(meta.data[, response.var, drop=FALSE])
      }
  }
  ...
  if(method=="omnibus"){
    ...
    for(i in 1:3){# take average of 3 runs to estimate stat.o
      ...
      feature.sel <- colnames(data)
      if(!is.null(adjust.vars))   response.adj -> data[, response.var]
  ...
  clusterExport(cl, c("ranger","calculateError")) 
  ...
  #once but not just passed via the arguments. Only variables in the MAIN environment (the 
  #environment where the function is executed) need to be exported, otherwise an error would
  #be thrown.
...
#      if (i %% 100 == 0) cat('.') 
#    }
    # permutate the response var, eliminating effects of covariates
    meta.perm<-meta.data
    if(!is.null(adjust.vars))   meta.perm[, response.var] <- sample(response.adj) else
