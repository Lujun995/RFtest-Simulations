...
randomForestTest_parallel_omnibus2 <- 
  function (comm, meta.data, response.var, status.var = NULL, adjust.vars = NULL, 
            ...
            presence.in = NULL,  test.type = c('Training', 'OOB', '0.632', '0.632+'),
  ...
  if(!is.null(presence.in)) {
    drop.otus.number <- which( colSums(1*(comm !=0)) < as.numeric(presence.in)*nrow(comm) )
  ...
  if(is.null(tree)) cum.mat<-comm else 
    if (class(tree) == "phylo") {
      cum.mat <- (t(calculate.cum(comm, tree)))
      cum.mat.pa <- (t(calculate.cum( (1*(comm != 0)), tree)))         
    } else
...
#  cat('\n')
  ...
  return(list(call = call, method = method, p.value.perm = p.value.perm, p.value.pearson = p.value.pearson, 
              pearson.para = pearson.para, stat.o = stat.o, w = w, drop.otus=drop.otus))
}
