  ...
  for(j in 1:length(n)){
    ...
    for(k in 1:length(signal)){
      for(l in 1:length(inter)){
        ...
        #RF omnibus test
        pv1 <- randomForestTest(comm = x , meta.data = data.frame(y=y[1:n[j],h,j,k,l]), tree=tree, 
                                                  response.var="y", perm.no = 999, n.cores=cores, test.type = "OOB",
												  method = "w", prediction.type = 'Regression',presence.in=1)
